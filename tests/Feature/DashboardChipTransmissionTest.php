...
/**
 * watchlistMovers / transmissionFocus 為 Dashboard 的 deferred props，不在初始 response，
 * 一律透過 getDashboard()（partial 請求）解析後以 json() 斷言。
 */
class DashboardChipTransmissionTest extends TestCase
{
    ...
    {
        $response = $this->actingAs($this->userWatching('2330.TW'))->getDashboard()->assertOk();

        $response->assertJsonCount(1, 'props.watchlistMovers');
        $chip = $response->json('props.watchlistMovers.0.chip');
        $this->assertIsArray($chip);

        foreach (['stance', 'days', 'foreign_net', 'foreign_streak', 'as_of'] as $key) {
            $this->assertArrayHasKey($key, $chip);
        }

        $this->assertArrayHasKey('alignment', $response->json('props.watchlistMovers.0'));
    }

    /** 美股沒有籌碼；chip 必須是 null，前端據此整列不顯示。 */
    public function test_us_mover_has_null_chip(): void
    {
        $response = $this->actingAs($this->userWatching('NVDA', 'Nvidia'))->getDashboard()->assertOk();

        $response->assertJsonCount(1, 'props.watchlistMovers');
        $response->assertJsonPath('props.watchlistMovers.0.chip', null);
    }
    ...
    public function test_transmission_focus_groups_news_by_chain(): void
    {
        ...
        $chains = $this->actingAs($this->userWatching('2330.TW'))->getDashboard()->assertOk()
    ...
    public function test_chains_without_hits_are_still_listed(): void
    {
        ...
        $chains = $this->actingAs($this->userWatching('NVDA', 'Nvidia'))->getDashboard()->assertOk()
            ->json('props.transmissionFocus');
    ...
    public function test_news_outside_the_lookback_window_is_excluded(): void
    {
        ...
        $this->actingAs($this->userWatching('2330.TW'))->getDashboard()->assertOk()
            ->assertJsonPath('props.transmissionFocus', []);
    }

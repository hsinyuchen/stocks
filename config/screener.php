...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | screener:warm 每天在這些時刻執行（形狀比照 config/news.php 與
    | config/youtube.php 的 schedule 區塊）。
    |
    | **這是全站唯一會主動刷新 daily_prices 的排程。** 在它存在之前，價格只在有人
    | 呼叫 dailyPrices() 時才抓，於是實測 67 檔有價格的標的裡有 31 檔停在 15–30 天
    | 前——早就過了 CachedMarketDataProvider::isFresh() 的 TTL，只是自從被批次拉
    | 進來後沒有任何人再碰過。技術面的新鮮度 gate（config/health.php 的 technical
    | 區塊）會把那些標的判成不可評估，而根治在這裡。
    |
    | 預設台北時間 16:00：台股 15:00 盤後才公佈完。美股當日資料要隔天才有，但那只有
    | 1 個交易日的年齡，遠在 gate 的門檻（8 個交易日）之內，不值得為它另排一次。
    |
    | **前提與既有排程相同**：部署環境要有每分鐘的 schedule:run 才會執行
    | （見 routes/console.php 的既有註解）。
    |
    */

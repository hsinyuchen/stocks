import { router, useForm } from '@inertiajs/react';
import { useEffect } from 'react';
import { Database, ExternalLink, KeyRound, Plus, Save, Star, Trash2 } from 'lucide-react';
import AppShell from '../../Layouts/AppShell';
import { useI18n } from '../../i18n';

// value→i18n key；顯示標籤在 render 時依當前語言解析。
const providerOptions = [
    { value: 'openai', labelKey: 'settings.providerOpenai' },
    { value: 'anthropic', labelKey: 'settings.providerAnthropic' },
    { value: 'gemini', labelKey: 'settings.providerGemini' },
    { value: 'openrouter', labelKey: 'settings.providerOpenrouter' },
    { value: 'deepseek', labelKey: 'settings.providerDeepseek' },
    { value: 'zeabur', labelKey: 'settings.providerZeabur' },
    { value: 'openai_compatible', labelKey: 'settings.providerOpenaiCompatible' },
    { value: 'ollama', labelKey: 'settings.providerOllama' },
    { value: 'llamacpp', labelKey: 'settings.providerLlamacpp' },
    { value: 'lmstudio', labelKey: 'settings.providerLmstudio' },
];

const providerHintKeys = {
    openai: 'settings.hintOpenai',
    anthropic: 'settings.hintAnthropic',
    gemini: 'settings.hintGemini',
    openrouter: 'settings.hintOpenrouter',
    deepseek: 'settings.hintDeepseek',
    zeabur: 'settings.hintZeabur',
    openai_compatible: 'settings.hintOpenaiCompatible',
    ollama: 'settings.hintOllama',
    llamacpp: 'settings.hintLlamacpp',
    lmstudio: 'settings.hintLmstudio',
};

const emptyForm = {
    provider_type: 'openai',
    display_name: '',
    base_url: '',
    api_key: '',
    model: '',
    timeout_seconds: 60,
    temperature: 0.2,
    max_tokens: 1200,
    is_default: false,
};

function FieldError({ message }) {
    if (!message) {
        return null;
    }

    return <p className="field-error">{message}</p>;
}

function TextField({ error, help, label, ...props }) {
    return (
        <label className="form-field">
            <span>{label}</span>
            <input {...props} />
            {help ? <small>{help}</small> : null}
            <FieldError message={error} />
        </label>
    );
}

function SelectField({ error, help, label, options, ...props }) {
    return (
        <label className="form-field">
            <span>{label}</span>
            <select {...props}>
                {options.map((option) => (
                    <option key={option.value} value={option.value}>
                        {option.label}
                    </option>
                ))}
            </select>
            {help ? <small>{help}</small> : null}
            <FieldError message={error} />
        </label>
    );
}

function DefaultToggle({ checked, disabled, onChange }) {
    const { t } = useI18n();

    return (
        <label className="settings-toggle">
            <input
                checked={checked}
                disabled={disabled}
                onChange={(event) => onChange(event.target.checked)}
                type="checkbox"
            />
            <span>{t('settings.setAsDefaultProvider')}</span>
        </label>
    );
}

function ProviderFormFields({ form, setting = null }) {
    const { t } = useI18n();
    const selectedProvider = form.data.provider_type;
    const providerOptionList = providerOptions.map((option) => ({ value: option.value, label: t(option.labelKey) }));

    return (
        <>
            <div className="settings-form-grid">
                <SelectField
                    error={form.errors.provider_type}
                    help={t(providerHintKeys[selectedProvider])}
                    label={t('settings.provider')}
                    onChange={(event) => form.setData('provider_type', event.target.value)}
                    options={providerOptionList}
                    value={selectedProvider}
                />
                <TextField
                    error={form.errors.display_name}
                    label={t('settings.displayName')}
                    maxLength="80"
                    onChange={(event) => form.setData('display_name', event.target.value)}
                    placeholder={t('settings.displayNamePlaceholder')}
                    type="text"
                    value={form.data.display_name}
                />
                <TextField
                    error={form.errors.model}
                    label={t('settings.modelName')}
                    maxLength="120"
                    onChange={(event) => form.setData('model', event.target.value)}
                    placeholder={t('settings.modelPlaceholder')}
                    type="text"
                    value={form.data.model}
                />
                <TextField
                    error={form.errors.base_url}
                    help={t('settings.baseUrlHelp')}
                    label="Base URL"
                    maxLength="255"
                    onChange={(event) => form.setData('base_url', event.target.value)}
                    placeholder="https://api.openai.com/v1"
                    type="url"
                    value={form.data.base_url}
                />
                <TextField
                    autoComplete="new-password"
                    error={form.errors.api_key}
                    help={t(setting?.has_api_key ? 'settings.apiKeyHelpSaved' : 'settings.apiKeyHelpNew')}
                    label="API Key"
                    maxLength="2048"
                    onChange={(event) => form.setData('api_key', event.target.value)}
                    placeholder={setting?.has_api_key ? t('settings.apiKeyPlaceholderSaved') : 'sk-...'}
                    type="password"
                    value={form.data.api_key}
                />
                <TextField
                    error={form.errors.timeout_seconds}
                    label={t('settings.timeoutSeconds')}
                    max="300"
                    min="5"
                    onChange={(event) => form.setData('timeout_seconds', event.target.value)}
                    type="number"
                    value={form.data.timeout_seconds}
                />
                <TextField
                    error={form.errors.temperature}
                    label="Temperature"
                    max="2"
                    min="0"
                    onChange={(event) => form.setData('temperature', event.target.value)}
                    step="0.01"
                    type="number"
                    value={form.data.temperature}
                />
                <TextField
                    error={form.errors.max_tokens}
                    label={t('settings.maxTokens')}
                    max="32000"
                    min="128"
                    onChange={(event) => form.setData('max_tokens', event.target.value)}
                    type="number"
                    value={form.data.max_tokens}
                />
            </div>
            <DefaultToggle
                checked={Boolean(form.data.is_default)}
                disabled={form.processing}
                onChange={(checked) => form.setData('is_default', checked)}
            />
        </>
    );
}

/**
 * FinMind 台股資料源 token（每人一組）。填自己的 token 用自己的免費額度抓台股行情/
 * 籌碼；不填則沿用全站預設。抓回的資料仍全站共用，token 只決定用誰的額度抓。
 */
function FinMindPanel({ finmind }) {
    const { t } = useI18n();
    const form = useForm({ token: '' });

    const submit = (event) => {
        event.preventDefault();
        form.post('/settings/finmind', {
            preserveScroll: true,
            onSuccess: () => form.reset('token'),
        });
    };

    const clear = () => {
        router.delete('/settings/finmind', { preserveScroll: true });
    };

    return (
        <form className="settings-panel" onSubmit={submit}>
            <div className="settings-panel__head">
                <div>
                    <p className="section-kicker">{t('settings.finmindKicker')}</p>
                    <h2>{t('settings.finmindHeading')}</h2>
                    <p>{t('settings.finmindIntro')}</p>
                    <p>
                        {t('settings.finmindNoTokenPrompt')}
                        {' '}
                        <a
                            href="https://finmindtrade.com/"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="settings-external-link"
                        >
                            {t('settings.finmindRegisterLink')}
                            <ExternalLink aria-hidden="true" size={14} />
                        </a>
                        {' '}
                        {t('settings.finmindRegisterSuffix')}
                    </p>
                </div>
                <button className="button-primary" disabled={form.processing} type="submit">
                    <Save aria-hidden="true" size={18} />
                    <span>{t('settings.saveToken')}</span>
                </button>
            </div>

            <div className="settings-form-grid">
                <TextField
                    autoComplete="new-password"
                    error={form.errors.token}
                    help={t(finmind?.has_token ? 'settings.finmindTokenHelpSaved' : 'settings.finmindTokenHelpNew')}
                    label="FinMind API Token"
                    maxLength="255"
                    onChange={(event) => form.setData('token', event.target.value)}
                    placeholder={t(finmind?.has_token ? 'settings.finmindTokenPlaceholderSaved' : 'settings.finmindTokenPlaceholderNew')}
                    type="password"
                    value={form.data.token}
                />
            </div>

            <div className="settings-provider__actions">
                <span className={`key-status ${finmind?.has_token ? 'key-status--active' : ''}`}>
                    <Database aria-hidden="true" size={15} />
                    {finmind?.has_token ? t('settings.finmindStatusSet') : t('settings.finmindStatusUnset')}
                </span>
                {finmind?.has_token ? (
                    <button className="icon-button icon-button--danger" onClick={clear} title={t('settings.clearTokenTitle')} type="button">
                        <Trash2 aria-hidden="true" size={18} />
                    </button>
                ) : null}
            </div>
        </form>
    );
}

function CreateProviderForm() {
    const { t } = useI18n();
    const form = useForm(emptyForm);

    const submit = (event) => {
        event.preventDefault();
        form.post('/settings', {
            preserveScroll: true,
            onSuccess: () => form.reset(),
        });
    };

    return (
        <form className="settings-panel" onSubmit={submit}>
            <div className="settings-panel__head">
                <div>
                    <p className="section-kicker">{t('settings.llmProviderKicker')}</p>
                    <h2>{t('settings.addProviderHeading')}</h2>
                    <p>{t('settings.addProviderIntro')}</p>
                </div>
                <button className="button-primary" disabled={form.processing} type="submit">
                    <Plus aria-hidden="true" size={18} />
                    <span>{t('settings.add')}</span>
                </button>
            </div>
            <ProviderFormFields form={form} />
        </form>
    );
}

function EditProviderForm({ setting }) {
    const { t } = useI18n();
    const form = useForm({
        provider_type: setting.provider_type,
        display_name: setting.display_name,
        base_url: setting.base_url ?? '',
        api_key: '',
        model: setting.model,
        timeout_seconds: setting.timeout_seconds,
        temperature: setting.temperature,
        max_tokens: setting.max_tokens,
        is_default: Boolean(setting.is_default),
    });

    const submit = (event) => {
        event.preventDefault();
        form.patch(`/settings/${setting.id}`, {
            preserveScroll: true,
            onSuccess: () => form.reset('api_key'),
        });
    };

    useEffect(() => {
        form.setData('is_default', Boolean(setting.is_default));
    }, [setting.is_default]);

    const destroy = () => {
        router.delete(`/settings/${setting.id}`, {
            preserveScroll: true,
        });
    };

    const makeDefault = () => {
        router.patch(`/settings/${setting.id}/default`, {}, {
            preserveScroll: true,
        });
    };

    return (
        <form className="settings-provider" onSubmit={submit}>
            <div className="settings-provider__head">
                <div className="settings-provider__title">
                    <strong>{setting.display_name}</strong>
                    <span>{(() => {
                        const matched = providerOptions.find((option) => option.value === setting.provider_type);
                        return matched ? t(matched.labelKey) : setting.provider_type;
                    })()}</span>
                </div>
                <div className="settings-provider__actions">
                    <span className={`key-status ${setting.has_api_key ? 'key-status--active' : ''}`}>
                        <KeyRound aria-hidden="true" size={15} />
                        {setting.has_api_key ? t('settings.keySaved') : t('settings.keyNone')}
                    </span>
                    {setting.is_default ? (
                        <span className="default-badge">
                            <Star aria-hidden="true" size={15} />
                            {t('settings.defaultBadge')}
                        </span>
                    ) : (
                        <button className="button-secondary" disabled={form.processing} onClick={makeDefault} type="button">
                            <Star aria-hidden="true" size={18} />
                            <span>{t('settings.setAsDefault')}</span>
                        </button>
                    )}
                    <button className="icon-button" disabled={form.processing} title={t('settings.saveProviderTitle')} type="submit">
                        <Save aria-hidden="true" size={18} />
                    </button>
                    <button className="icon-button icon-button--danger" onClick={destroy} title={t('settings.deleteProviderTitle')} type="button">
                        <Trash2 aria-hidden="true" size={18} />
                    </button>
                </div>
            </div>
            <ProviderFormFields form={form} setting={setting} />
        </form>
    );
}

export default function SettingsIndex({ settings = [], finmind = { has_token: false, updated_at: null } }) {
    const { t } = useI18n();

    return (
        <AppShell title={t('settings.pageTitle')}>
            <div className="settings-page">
                <FinMindPanel finmind={finmind} />

                <CreateProviderForm />

                <section className="settings-stack" aria-label={t('settings.savedProvidersRegion')}>
                    {settings.length > 0 ? (
                        settings.map((setting) => (
                            <EditProviderForm key={setting.id} setting={setting} />
                        ))
                    ) : (
                        <div className="settings-provider empty-state">
                            <strong>{t('settings.emptyTitle')}</strong>
                            <span>{t('settings.emptyHint')}</span>
                        </div>
                    )}
                </section>
            </div>
        </AppShell>
    );
}
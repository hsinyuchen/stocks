...
// 分組與排序用的常數。兩個層級的順序是「關聯由硬到軟」，方向的 `none` 排最後
// ——那是「傳導表沒標」，不是第三種方向。
const TIER_ORDER = ['core', 'extended'];
 ...
 * 依 `tier` × `direction` 分組。
 *
 * **只分組，不重算任何判定。** 層級與方向都是後端解好的欄位，前端再篩一次會讓
 * 同一份資料被兩把尺量出互相矛盾的分類。兩者都用**物件鍵**而不是相等比較，
 * 連「順手加個條件」的空間都不留。
 *
 * `direction` 為 null 併進 `none` 桶：`rate_policy` 這種 sector 全宣告 neutral
 * 的題材連核心都沒有方向，「無方向」是傳導表沒標，不是某一層的專屬狀態。
 */
 ...
 * 傳導假設、同產業推導、營收驗證三種完全不同強度的證據排在同一張清單上，使用者
 * 沒有這四句話就無從判斷每一列各代表什麼。放進摺疊或只在選了題材後才顯示，等於
...
function TopicBoardView({ board, groups }) {
    ...
    return (
        <div className="topic-board">
            ...
            {/* 空清單走得到：延伸層是從核心推導的，所以 board 空掉只有一個成因
                ——傳導表沒有列出任何個股，或列出的全是指數／ETF（非個股一律不列，
                見 TopicCandidateResolver::nonStockSymbols）。文案只講這個真實成因
                ——列名的標的不會因為「沒有資料」被過濾掉。 */}
            {groups.length === 0 ? (
                <p className="dashboard-empty">{t('topics.emptyCandidates')}</p>
            ) : (
                groups.map((group) => (
                    <section className="topic-group" key={group.key}>
                        ...
                        <ul className="topic-candidates">
                            {group.rows.map((candidate) => (
                                <CandidateRow candidate={candidate} key={candidate.symbol} />
                            ))}
                        </ul>
                    </section>
                ))
